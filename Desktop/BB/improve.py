...
def fetch_poster(movie_id):
    response = requests.get('https://api.themoviedb.org/3/movie/{}?api_key=9988968e96c3f885affd539d47592358&language=en-US'.format(movie_id))
    data = response.json()
    return "https://image.tmdb.org/t/p/w500/" + data['poster_path']
def recommend(movie):
    movie_index = movies[movies['title'] == movie].index[0]  # 1
    distances = similarity[movie_index]  # 2
    movies_list = sorted(list(enumerate(distances)), reverse=True, key=lambda x: x[1])[1:6]  # 3
...
if selected_movie_name:
    ...
    st.image(selected_movie_poster, width=180)  # Adjust the width to your preference
